...
class CV_evaluation:
    ...
    def cv_nn_evaluate(self):
        ...
        if t_pp:
            ...
            #save the threshold for threshold post processing
            threshold_set = []
        if a_pp:
            metrics_average = label_matching.result_analysis(0, 'hit_match')
            #save thre threshold and average step for average post processing
            threshold_average_set = []
        if h_b_pp:
        ...
        for i in range(K_FOLD):
            ...
            if verbose:
                print('predict begin')
            nn_predict = nn_function.model_predict(model_p, pred_seq_len = data_p.audio_len, seed = 42, model_path =  model_path + str(i) + '.pt')
            ...
            if t_pp:
                threshold_processsing = post_process.threshold_post_process()
                threshold_processsing(nn_predict.train_prob_predict, nn_predict.val_prob_predict, nn_predict.test_prob_predict)
                metrics_threshold(threshold_processsing.test_bino_predict['label'], threshold_processsing.test_bino_predict['pred'])
                threshold_set.append(threshold_processsing.best_threshold)
                if h_b_t_pp:
                    hmm_threshold_postprocessing = post_process.hmm_post_process()
                    hmm_threshold_postprocessing.predict_hmm_bino(threshold_processsing.train_bino_predict, threshold_processsing.val_bino_predict, threshold_processsing.test_bino_predict)
            ...
            if a_pp:
                average_processing = post_process.average_postprocess(n_jobs = 6)
                average_processing(nn_predict.train_prob_predict, nn_predict.val_prob_predict, nn_predict.test_prob_predict)
                metrics_average(average_processing.test_bino_predict['label'], average_processing.test_bino_predict['pred'])
                threshold_average_set.append((average_processing.best_threshold, average_processing.best_average_step))
            if h_b_pp or h_g_pp:
                hmm_post_processing = post_process.hmm_post_process()
                if h_b_pp:
                    hmm_post_processing.predict_hmm_bino(nn_predict.train_bino_predict, nn_predict.val_bino_predict, nn_predict.test_bino_predict)
                    metrics_hmm_bino(hmm_post_processing.hmm_bino_pred_label['label'], hmm_post_processing.hmm_bino_pred_label['pred'])
                if h_g_pp:
                    hmm_post_processing.predict_hmm_gmm(nn_predict.train_raw_predict, nn_predict.val_raw_predict, nn_predict.test_raw_predict)
                    metrics_hmm_gmm(hmm_post_processing.hmm_gmm_pred_label['label'], hmm_post_processing.hmm_gmm_pred_label['pred'])
                    mix_set.append(hmm_post_processing.best_mix_number)
        ...
        if t_pp:
            ...
            self.threshold_result['threshold_set'] = threshold_set
            if verbose:
        ...
        if a_pp:
            ...
            self.average_result['threshold_averagestep_set'] = threshold_average_set
            if verbose:
        ...
        if h_g_pp:
            ...
            self.hmm_gmm_result['gmm_mix_set'] = mix_set
            if verbose:
