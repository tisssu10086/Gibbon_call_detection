...
class hmm_post_process:
    def __init__(self, hidden_state_number: int = 2, observe_state_number: int = 2):

        self.hidden_state_number = hidden_state_number
        self.observe_state_number = observe_state_number
        self.emission_matrix = None
        self.emission_dist = None
        self.best_mix_number = None
    ...
    def predict_hmm_bino(self, train_bino_predict_label, val_bino_predict_label, test_bino_predict_label):
        num_state = self.hidden_state_number
        ...
        self.transition_matrix = self.cal_tr_matrix(train_val_label)
        self.pi_vector = self.cal_pi_vector(train_val_label)
        self.emission_matrix = self.cal_em_matrix(train_val_predict, train_val_label)

        model = hmm.MultinomialHMM(n_components=num_state)
        model.startprob_= self.pi_vector
    ...
    def predict_hmm_gmm(self, train_raw_predict_label, val_raw_predict_label, test_raw_predict_label):
        num_state = self.hidden_state_number
        ...
        for num_mix in NUM_MIX:
            different_mix_result.append(self.cal_em_dist(train_val_predict, train_val_label, num_mix))
            different_mix_bic.append(self.cal_em_dist(train_val_predict, train_val_label, num_mix)['bic'])
        #find the mixture with minimum bic 
        best_index = different_mix_bic.index(min(different_mix_bic))
        self.best_mix_number = NUM_MIX[best_index]
        self.emission_dist = different_mix_result[best_index]


        model = hmm.GMMHMM(n_components=num_state, n_mix= self.emission_dist['num_mix'])
        model.startprob_= self.pi_vector
        model.transmat_= self.transition_matrix
        model.weights_= self.emission_dist['weights']
        model.means_ = self.emission_dist['means']
        model.covars_ = self.emission_dist['covs']
...
class average_postprocess:
    ...
    def __call__(self, train_prob_predict_label, val_prob_predict_label, test_prob_predict_label):
        ...
        self.best_average_step = best_average_step
