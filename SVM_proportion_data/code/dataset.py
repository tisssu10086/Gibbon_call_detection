...
class Hparams:
    ...
    mfcc_fft_seq_len = 256
    mfcc_fft_hop_len = 128
    mfcc_n_mels = 40
    mfcc_f_min = 1000
    mfcc_f_max = 2000
    n_mfcc = 25
...
class MFCC_transform(Transforms):
    def __init__(self, cfg):
        self.cfg = cfg


    def generate_mfcc_stat(self, audio):
        """This function is to convert sound track to mfcc features"""
        n_fft = self.cfg.mfcc_fft_seq_len
        hop_length = self.cfg.mfcc_fft_hop_len
        n_mels = self.cfg.mfcc_n_mels
        f_min = self.cfg.mfcc_f_min
        f_max = self.cfg.mfcc_f_max
        sr = self.cfg.sample_rate   
        n_mfcc = self.cfg.n_mfcc

        mel_spec = librosa.feature.melspectrogram(audio, n_fft=n_fft, hop_length=hop_length, n_mels=n_mels, sr= sr, power= 2.0, fmin = f_min, fmax=f_max)
        log_mel_spec = librosa.power_to_db(mel_spec)    

        mfcc =  librosa.feature.mfcc(S = log_mel_spec, n_mfcc = n_mfcc)
        mfcc_delta = librosa.feature.delta(mfcc, width = 3, order = 1, axis = 0, mode = 'nearest')
        mfcc_accle = librosa.feature.delta(mfcc_delta, width = 3, order = 1, axis = 0, mode = 'nearest')

        min_mfcc = np.min(mfcc, axis = -1)
        max_mfcc = np.max(mfcc, axis = -1)
        med_mfcc = np.median(mfcc, axis = -1)
        mean_mfcc = np.mean(mfcc, axis = -1)
        var_mfcc = np.var(mfcc, axis = -1)
        skew_mfcc = scp.stats.skew(mfcc, axis = -1)
        kurt_mfcc = scp.stats.kurtosis(mfcc, axis = -1)

        mean_mfcc_delta = np.mean(mfcc_delta, axis = -1)
        var_mfcc_delta = np.var(mfcc_delta, axis = -1)
        mean_mfcc_accle = np.mean(mfcc_accle, axis = -1)
        var_mfcc_accle = np.var(mfcc_accle, axis = -1)

        mfcc_stat = np.concatenate((min_mfcc, max_mfcc, med_mfcc, mean_mfcc, var_mfcc, skew_mfcc, kurt_mfcc, mean_mfcc_delta, var_mfcc_delta, mean_mfcc_accle, var_mfcc_accle))
        return mfcc_stat
    ...
    def __call__(self, audios: np.ndarray, data_name: Path, dataset_usage: str)-> np.ndarray:
        ...
        if dataset_usage == 'overlap_train':
            save_dic = Path('overlap_data/mfcc_data')
        elif dataset_usage == 'nonoverlap_pred':
            save_dic = Path('nonoverlap_data/mfcc_data')
        else:
        ...
        if total_save_path.exists() and not overwritten:
            # print('load mfcc data')
            mfcc_data = pickle.load(open(total_save_path, 'rb'))
        else:
            mfcc_data = Parallel(n_jobs= n_jobs)(delayed(self.generate_mfcc_stat)(audio) for audio in audios)
            mfcc_data = np.asarray(mfcc_data)
            pickle.dump(mfcc_data, open(total_save_path, 'wb'))
        return mfcc_data
...
def cross_valid(seed, split_number, fold_needed, file_dic: list, save_path: Path, overwritten: bool = False, 
                verbose: bool = False, train_val_proportion: float = 1, valid_proportion: float = 0.3):
    ...
    for train_index, test_index in kf.split(file_dic):
        ...
        train_val_length = train_val_set.shape[0]
        #decide the proportion of data for training and validation
        train_val_length_p = np.ceil(train_val_length * train_val_proportion).astype(int)
        train_val_set = train_val_set[: train_val_length_p]
        val_length = np.ceil(train_val_length_p * valid_proportion).astype(int)
        train_set = train_val_set[:-val_length]
        val_set = train_val_set[-val_length:]
